...
import '../../support/controller/support_chat_controller.dart';
...
class NotificationController extends GetxController {
  ...
  hitNotificationsListApiCall({page}) async {
    ...
    APIRepository.notificationListCall(page: page).then((value) async {
      customLoader.hide();
      if (value != null) {
        notificationListResponseModel = value;
        if (page == 0) {
          notificationsList.value = notificationListResponseModel?.data ?? [];
        } else {
          notificationsList.addAll(notificationListResponseModel?.data ?? []);
        }
  ...
  hitClearAllNotificationAPI() async {
    APIRepository.clearAllNotificationCall().then((value) async {
      if (value != null) {
  ...
  notificationRedirection(Map<String, dynamic> data) async {
    ...
    if (typeId == constNotifyChatSupport &&
      ...
      if (Get.isRegistered<SupportChatController>()) {
        Get.find<SupportChatController>().onReady();
      } else {
  ...
  void _navigateToChatScreen({
    ...
    if (Get.currentRoute == AppRoutes.chatRoute) {
      if (Get.isRegistered<ChatController>()) {
        if (Get.find<ChatController>().fromId == id) {
          //chatting with same person
          return;
        } else {
          Get.find<ChatController>().fromId = id;
          Get.find<ChatController>().userName.value = name;
          Get.find<ChatController>().userImage.value = profilePicUrl;
          Get.find<ChatController>().hitLoadChatApiCall(isFirst: true);
        }
      }
      return;
    } else {
