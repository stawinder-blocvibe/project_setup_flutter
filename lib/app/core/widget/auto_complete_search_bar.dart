...
import '../utils/debouncer_utility.dart';
import '../utils/places_api_provider.dart';
...
class _AutocompleteSearchBarState extends State<AutocompleteSearchBar> {
  ...
  Future<List<Suggestion>?> _search(String query) async {
    ...
    final List<Suggestion> options = await _placeApi!.fetchSuggestionsNew(
        _currentQuery!, Localizations.localeOf(context).languageCode);

    debugPrint('options--->${_currentQuery}${options}');
        ...
        suggestionsBuilder:
          ...
          _lastOptions = List<ListTile>.generate(options.length, (int index) {
            ...
            return ListTile(
                ...
                onTap: () async {
                  debugPrint('clicked--->${item.placeId}');
                  LatLng latLng =
                  ...
                  widget.onSuggestionSelected(
                      {'latLng': latLng, 'description': item.description});
